...
$.ajax({url:baseurl,type:"POST",data:data,beforeSend:function(){$('body').append('<div class="div-loading"></div>');},success:function(response){console.dir(response);$(".div-loading").remove();self.isEditing(false);self.parent.isEditing(false);}})}
ko.mapping.fromJS(data,{},self);}
...
if(file.size>5000000){alert("File is too big");}else if(file.type!='application/pdf'){alert("File is not a pdf file");}}}else{var form=new FormData(document.getElementById('resume-submit'));var baseurl=getAjaxUrl()+"submit_resume";form.append('student_number',self.student_number());$.ajax({type:'POST',url:baseurl,data:form,beforeSend:function(){$('body').append('<div class="div-loading"></div>');},processData:false,contentType:false,success:function(response){if(response){$(".div-loading").remove();$("#submit-success").fadeIn("fast");}else{$(".div-loading").remove();$("#submit-success").fadeIn("fast");}},error:function(x,t,h){$(".div-loading").remove();$("#submit-error").fadeIn("fast");},complete:function(){$('form#resume-submit').find('input').removeAttr('checked');$('form#resume-submit').find('userfile').val('');}});return false;}}}
...
{item.cancel(item);}else{var $mask=$('#extra-curricular .alert-mask:eq('+index+')');var $alert=$('#extra-curricular .alert-changes:eq('+index+')');$mask.on('click',function(){$mask.removeClass('alert-notify');$alert.removeClass('alert-notify');});editor.scroll();editor.scrollTo($alert);failed+=1;setTimeout(function(){$mask.addClass('alert-notify');$alert.addClass('alert-notify');},100);setTimeout(function(){$mask.removeClass('alert-notify');$alert.removeClass('alert-notify');},2500);return false;}}});}else if(self.isSkills()){self.skills.cancel();}
return failed==0;}
