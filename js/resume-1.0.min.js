var PersonalInfoModel=function(data,parent){var self=this;self.parent=parent;self.first_name=ko.observable("");self.last_name=ko.observable("");self.address=ko.observable("");self.email_address=ko.observable("");self.mobile_number=ko.observable("");self.student_number=ko.observable(parent.student_number());self.isEditing=ko.observable(false);self.old_data={};function getRequiredFields(){var required_fields={first_name:self.first_name(),last_name:self.last_name(),address:self.address(),email_address:self.email_address(),mobile_number:self.mobile_number()}
...
self.edit=function(){self.setOldData();self.isEditing(true);}
...
self.cancel=function(){self.isEditing(false);self.closeInfo();}
...
self.save=function(item,event){var required_fields=self.validateFields();console.dir(required_fields);if(!required_fields){return false;}
if(self.hasNoChanges()){}else{var $target=$(event.target);var $personalinfo=$target.parents('div.data-group');data={data:required_fields,student_number:self.student_number(),section:"personal_information"}
$.ajax({url:"resume_editor/save_personal_information",method:"POST",data:data,beforeSend:function(){$('body').append('<div class="div-loading"></div>');},success:function(response){if(response){var response=JSON.parse(response);if(response['code']==1){$(".div-loading").remove();}else{window.location.replace(response['url']);}
...
if(!failed){var data;data={section:"education",type:self.type(),student_number:self.parent.student_number(),data:self.getMutatedMainFields(),recognitions:self.getRecognitions(),delete_recognitions:self.to_delete_recognitions};$.ajax({url:"resume_editor/save_education",method:"POST",data:data,beforeSend:function(){$('body').append('<div class="div-loading"></div>');},success:function(response){if(response){response=JSON.parse(response);if(response['code']==404){console.dir(response);}
...
$('div.div-loading').remove();$education.find('.alert-mask').removeClass(".alert-notify");$education.find('.alert-info').removeClass(".alert-notify");self.isEditing(false);self.parent.isEditing(false);}},error:function(x,t,h){$('div.div-loading').remove();setTimeout(function(){$work.find('.alert-mask').addClass("alert-notify");$work.find('.alert-error').addClass("alert-notify");},300);$work.find('.alert-mask').on('click',function(){$work.find('.alert-mask').removeClass('alert-notify');$work.find('.alert-error').removeClass('alert-notify');});},timeout:120000});}}
...
self.edit=function(item,event){self.isEditing(true);self.parent.isEditing(true);self.old_data=self.getMainFields();self.old_recognitions([]);self.to_delete_recognitions=[];ko.utils.arrayForEach(self.recognitions(),function(a){var b={};b.award_id=a.award_id();b.award=a.award();self.old_recognitions.push(new RecognitionModel(b,self));});if(event!=undefined){var offsetToScroll=$(event.target).parents('div.education-group').position().top;if(offsetToScroll>200){editor.scroll();editor.animate({scrollTop:offsetToScroll-100},"");}}}
...
self.remove=function(item,event){$target=$(event.target);self.closeInfo();var $work=$target.parents('div.work-experience-group');if(self.work_id()){$.ajax({url:"resume_editor/delete_work_experience",method:"POST",data:{section:"work_experience",student_number:self.parent.student_number(),work_id:self.work_id()},beforeSend:function(){$('body').append('<div class="div-loading"></div>');},success:function(response){if(response){response=JSON.parse(response);if(response['code']==1||response['code']==0){self.parent.workExp.remove(self);self.parent.isEditing(false);}else{window.location.replace(response['url']);}}},complete:function(){setTimeout(function(){$('div.div-loading').remove();self.isEditing(false);self.parent.isEditing(false);},800);}});}else{self.parent.workExp.remove(self);self.parent.isEditing(false);}}
...
self.parent.workExp().sort(function(l,r){return l.from_date()<r.from_date()?1:-1});self.parent.workExp.valueHasMutated();$('div.div-loading').remove();$work.find('.alert-mask').removeClass(".alert-notify");$work.find('.alert-info').removeClass(".alert-notify");self.isEditing(false);self.parent.isEditing(false);}},error:function(x,t,h){$work.find('div.div-loading').remove();$mask=$work.find('.alert-mask');$alert=$work.find('.alert-error');var offsetToScroll=$alert.offset().top;editor.scroll();editor.animate({scrollTop:offsetToScroll+300},"");setTimeout(function(){$mask.addClass("alert-notify");$alert.addClass("alert-notify");},300);$work.find('.alert-mask').on('click',function(){$work.find('.alert-mask').removeClass('alert-notify');$work.find('.alert-error').removeClass('alert-notify');});}});}}
...
self.edit=function(item,event){self.isEditing(true);self.parent.isEditing(true);self.old_data=self.getMainFields();self.old_work_descriptions=[];ko.utils.arrayForEach(self.work_descriptions(),function(a){var b={};b.work_description_id=a.work_description_id();b.work_description=a.work_description();self.old_work_descriptions.push(b);});if(event!=undefined){var offsetToScroll=$(event.target).parents('div.work-experience-group').offset().top;editor.scroll();editor.animate({scrollTop:offsetToScroll-200},"");}}
...
self.addCommittee=function(){var count=self.committees().length;if(count<10)
...
failed+=1;}};ko.utils.arrayForEach(self.committees(),function(committee,index){failed+=committee.validate($affiliation,index);});if(!failed){var data;data={section:"affiliation",affiliation_id:self.affiliation_id(),student_number:self.parent.student_number(),data:{affiliation_name:self.affiliation_name(),student_number:self.parent.student_number()},committees:self.getCommittees(),delete_committees:self.to_delete_committees};$.ajax({url:"resume_editor/save_affiliation",method:"POST",data:data,beforeSend:function(){$('body').append('<div class="div-loading"></div>');},success:function(response){if(response){response=JSON.parse(response);if(response['code']==404){window.location.replace(response['url']);}
...
self.committees().sort(function(l,r){return l.from_date()<r.from_date()?1:-1});self.committees.valueHasMutated();$('div.div-loading').remove();$affiliation.find('.alert-mask').removeClass(".alert-notify");$affiliation.find('.alert-info').removeClass(".alert-notify");self.isEditing(false);self.parent.isEditing(false);}},error:function(x,t,h){$('div.div-loading').remove();$mask=$affiliation.find('.alert-mask');$alert=$affiliation.find('.alert-error');var offsetToScroll=$alert.offset().top;editor.scroll();editor.animate({scrollTop:offsetToScroll+300},"");setTimeout(function(){$affiliation.find('.alert-mask').addClass("alert-notify");$affiliation.find('.alert-error').addClass("alert-notify");},300);$affiliation.find('.alert-mask').on('click',function(){$affiliation.find('.alert-mask').removeClass('alert-notify');$affiliation.find('.alert-error').removeClass('alert-notify');});},timeout:120000});}}
self.remove=function(item,event){$target=$(event.target);var $affiliation=$target.parents('div.affiliation-group');if(self.affiliation_id()){$.ajax({url:"resume_editor/delete_affiliation",method:"POST",data:{section:"affiliation",student_number:self.parent.student_number(),affiliation_id:self.affiliation_id()},beforeSend:function(){$('body').append('<div class="div-loading"></div>');},success:function(response){if(response){response=JSON.parse(response);if(response['code']==1||response['code']==0){self.parent.affiliations.remove(self);self.parent.isEditing(false);}else{window.location.replace(response['url']);}}},complete:function(){setTimeout(function(){$('div.div-loading').remove();self.isEditing(false);self.parent.isEditing(false);},800);}});}else{self.parent.affiliations.remove(self);self.parent.isEditing(false);}}
...
self.getCommittees=function(){var committees=[];ko.utils.arrayForEach(self.committees(),function(item){var data={};var committee={};committee['committee_id']=item.committee_id();data['committee_name']=item.committee_name();data['from_month']=item.from_month();data['from_year']=item.from_year();data['to_month']=item.to_month();data['to_year']=item.to_year();data['task_description']=item.task_description();committee['data']=data;committees.push(committee);});return committees;}
...
notempty++;}
ko.utils.arrayForEach(self.committees(),function(committee){var required={committee_name:committee.committee_name(),from_month:committee.from_month(),from_year:committee.from_year(),to_month:committee.to_month(),to_year:committee.to_year(),task_description:committee.task_description()}
for(var prop in required){var str="";var b=required[prop];var match=str===b;if(!match)
notempty++;}});return notempty==0;}
...
return true;}
ko.mapping.fromJS(data,mapping,self);}
var CommitteeModel=function(data,parent){var self=this;self.parent=parent;self.committee_id=ko.observable("");self.committee_name=ko.observable("");self.from_date=ko.observable("");self.from_month=ko.observable("");self.from_year=ko.observable("");self.to_month=ko.observable("");self.to_year=ko.observable("");self.task_description=ko.observable("");self.remove=function(){if(self.parent.committees().length>1)
if(self.committee_id())
self.parent.to_delete_committees.push(self.committee_id());self.parent.committees.remove(self);}
self.getMainFields=function(){var required={committee_name:self.committee_name(),from_month:self.from_month(),from_year:self.from_year(),to_month:self.to_month(),to_year:self.to_year(),task_description:self.task_description()}
return required;}
self.validate=function($affiliation,index){var $committeegroup=$affiliation.find('.committee-group:eq('+index+')');var failed=0;var required={committee_name:self.committee_name(),from_month:self.from_month(),from_year:self.from_year(),to_month:self.to_month(),to_year:self.to_year(),task_description:self.task_description()}
...
failed+=1;}};return failed;}
ko.mapping.fromJS(data,mapping,self);}
var offsetToScroll="";var editor=$("div#grid-editor");var rules={"default":/^[ñA-Za-z0-9 \-\.']+$/i,"student_number":/^[0-9]{9}$/,"first_name":/^[ñA-Za-z0-9 \-\.']+$/i,"last_name":/^[ñA-Za-z0-9 \-\.']+$/i,"mobile_number":/^[0-9]{11}$/,"email_address":/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,4})$/,"address":/^(?!\s*$).+/,"year":/^[12][0-9]{3}$/}
...
return false;}
self.addWorkExp=function(){var count=self.workExp().length;if(count<10&&!self.isEditing()){var work=new WorkExpModel({work_id:0},self);self.workExp.push(work);work.edit(work);self.isEditing(true);$('[data-toggle="tooltip"]').tooltip();var offsetToScroll=$('div.work-experience-group').last().offset().top;editor.scroll();editor.animate({scrollTop:offsetToScroll-200},"");}}
self.addAffiliation=function(){var count=self.affiliations().length;if(count<10&&!self.isEditing()){self.affiliations.push(new AffiliationModel({affiliation_id:0,isEditing:true},self));self.isEditing(true);var offsetToScroll=$('div.affiliation-group').last().offset().top;editor.scroll();editor.animate({scrollTop:offsetToScroll-200},"");}}
...
self.isCertified=ko.observable(false);self.submitResume=function(){if(self.validateSubmit()){if(!self.isUndergraduate()){self.application_type(1);}
...
if(!self.application_type()){$(".application-type").addClass("terms-danger");setTimeout(function(){$(".application-type").removeClass("terms-danger");},1200);}}else{$.ajax({url:"resume_editor/submit_resume",method:"POST",dataType:'html',data:{student_number:self.student_number(),application_type:self.application_type(),student_type:self.student_type()},beforeSend:function(){$('body').append('<div class="div-loading"></div>');},success:function(response){if(response){$(".div-loading").remove();$("#submit-success").fadeIn("fast");}},complete:function(){}});}}}
...
if(valid){$.ajax({type:"POST",url:"resume_editor/save_item",data:data,beforeSend:function(){$('body').append('<div class="div-loading"></div>');},success:function(response){if(!response)
...
for(var i in prop){var count=0;for(var j in prop[i]){$field=$("#education").find('input[name~="'+i+prop[i][j]+'"], textarea[name~="'+i+prop[i][j]+'"], select[name~="'+i+prop[i][j]+'"]');if($field.length==0)
...
failed+=1;});}
if(failed){var offsetToScroll=$('.danger').first().offset().top;editor.scroll();editor.animate({scrollTop:offsetToScroll-300},"");}else{self.cancelEmptyItems();}
return failed==0;}
...
ko.bindingHandlers.fadeVisible={init:function(element,valueAccessor){var value=valueAccessor();$(element).toggle(ko.unwrap(value));},update:function(element,valueAccessor){var value=valueAccessor();ko.unwrap(value)?$(element).delay(50).fadeIn(400):$(element).fadeOut(50);}};}
