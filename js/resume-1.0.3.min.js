...
{item.cancel(item);}else{var $mask=$('#extra-curricular .alert-mask:eq('+index+')');var $alert=$('#extra-curricular .alert-changes:eq('+index+')');$mask.on('click',function(){$mask.removeClass('alert-notify');$alert.removeClass('alert-notify');});editor.scroll();editor.scrollTo($alert);failed+=1;setTimeout(function(){$mask.addClass('alert-notify');$alert.addClass('alert-notify');},100);setTimeout(function(){$mask.removeClass('alert-notify');$alert.removeClass('alert-notify');},2500);return false;}}});}else if(self.isSkills()){if(self.skills.isEditing())
self.skills.cancel();}
return failed==0;}
